        ...
        if data.shape[1] == 2:
            ...
            if area is None:
                ...
                ax.scatter(
                    ...
                    data[~noise_mask][:, 1],
                    c=hashColor(y[~noise_mask]),
                    s=25 * area[~noise_mask].astype(np.float64) ** 3,
                    alpha=0.5,
                )
                ax.scatter(
                    data[noise_mask][:, 0],
                    data[noise_mask][:, 1],
                    c=hashColor(y[noise_mask]),
                    s=area[noise_mask].astype(np.float64),
                    alpha=0.1,
                )
            plt.axis("equal")
...
def autoPlotly(X, labels):
    ...
    fig = go.Figure(
        data=go.Scatter(
            ...
            mode="markers",
            marker=dict(
                color=[named_colors[int(i) % 24] for i in labels], line_width=1
            ),
            hoverinfo="text",
        ...
        while idx < len(tasks):
            ...
            for pid in range(pnum):
                if Process_state[str(pid)] == True:
                    ...
                    p = Process(
                        target=self.callback, args=(tasks[idx], Y, pid, Process_state)
                    )
                    print("{}".format(idx))
                    idx += 1
                    p.start()
